\begin{titlepage}

    % Colocar la imagen de fondo
    \begin{tikzpicture}[remember picture, overlay]
        \node[opacity=0.03, anchor=south east, xshift=-1cm, yshift=0.5cm] at (current page.south east) {
            \includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{images/logo-fondo.png}
        };
    \end{tikzpicture}

    \begin{tikzpicture}[remember picture, overlay]
        % Dibujar las dos líneas verticales en el lado izquierdo y menos largas
        \draw[thick] (current page.north west) ++(1cm,-2cm) -- ++(0,-24cm);
        \draw[thick] (current page.north west) ++(1.2cm,-2cm) -- ++(0,-24cm);
    \end{tikzpicture}
    
    \begin{center}
        % Logos alineados y centrados
        \begin{minipage}{2.5cm}
            \begin{center}
                \includegraphics[width=3.0cm]{images/logo-sep.png}
            \end{center}
        \end{minipage}\hfill
        \begin{minipage}{10cm}
            \begin{center}
                {\large \textbf{TECNOLÓGICO NACIONAL DE MÉXICO}}\\[0.2cm]
                {\LARGE \textbf{INSTITUTO TECNOLÓGICO}\\[0.1cm] \textbf{DE OAXACA}}\\[0.2cm]
                {\normalsize \textbf{\textit{"Tecnología Propia e Independencia Económica"}}}
            \end{center}
        \end{minipage}\hfill
        \begin{minipage}{2.5cm}
            \begin{center}
                \includegraphics[width=3.5cm]{images/logo-ito}
            \end{center}
        \end{minipage}

        \rule[1cm]{\textwidth}{3pt}

        % Encabezado
        \vspace{-0.5cm}
        \LARGE
        \underline{\textbf{\myAsignature}} \\[0.1cm]
        \small
        \myClave

        % Carrera
        \vspace{0.8cm}
        \large
        Carrera: \textbf{INGENIERÍA EN SISTEMAS COMPUTACIONALES}
        
        % Actividad destacada entre dos líneas
        \vspace{0.8cm}
        \begin{center}
            \rule{\textwidth}{0.5pt} \\[0.4cm]
            %{\large Actividad} \\[0.4cm]
            \textit{\descripcion} \\ [0.4cm]
            \textbf{"\myActivity"} \\[0.4cm]
            \rule{\textwidth}{0.5pt}
        \end{center}
        \vspace{0.4cm}


        % Datos del alumno y docente
        \vspace{0.8cm}
        \begin{minipage}{0.48\textwidth}
            \begin{flushleft}
                {\large
                    Presenta:\\    
                    \textbf{\authora}\\
                    %\textbf{\authorb}\\
                    %\textbf{\authorc}\\
                    %\textbf{\authord}\\
                    Grupo: \textbf{\myGroup} | Matrícula: \textbf{\myControlNumber}
                }
            \end{flushleft}
        \end{minipage}
        \hfill
        \begin{minipage}{0.48\textwidth}
            \begin{flushright}
                {\large
                    Docente:\\
                    \textbf{\myReviewers}
                }
            \end{flushright}
        \end{minipage}
        
        \vfill
        
        % Fecha
        {\large Periodo: \myPeriod}\\
        \vspace{0.3cm}
        {\large \myDate}
        
    \end{center}
\end{titlepage}
